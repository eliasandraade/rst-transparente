...
export default function PublicarParecerButton({ id, status }: Props) {
  ...
  async function handleClick() {
    setCarregando(true);
    try {
      await fetch(`/api/pareceres/${id}/status`, {
        method: "PATCH",
  ...
  return (
    <button
      ...
      aria-label={isPublicado ? "Despublicar parecer" : "Publicar parecer"}
    >
      {carregando ? (
